...
export default function DataTableOrders<Entry extends {}>(
  ...
  const handleExportPDF = () => {
    ...
    const tableRows =
      orders?.list?.map((row: any, index: number) => { // Adding index for numbering
        const orderItems = row.products
          ? row.products
      ...
      <div className="panel">
        <div className="flex flex-col md:flex-row items-center justify-between mb-6 gap-4 p-4 bg-white dark:bg-slate-900 shadow rounded-lg">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button onClick={handleExportPDF} className="btn btn-primary btn-sm flex items-center gap-2">
            <IconFile className="w-5 h-5" />
            <span>PDF</span>
            </button>
